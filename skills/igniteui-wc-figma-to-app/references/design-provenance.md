...
> [figma-component-map.md](figma-component-map.md) resolves to an Ignite UI tag. Read it
...
| Figma Layer | Tier | Kit / source | Canonical role + props | Ignite UI tag | Confidence | Anatomy deltas |
| --- | --- | --- | --- | --- | --- | --- |
| `Button` (`Variant=outline, Size=sm`) | B | shadcn/ui | `button` · medium · 32px | `<igc-button variant="outlined">` | high | Label casing, radius → tokens |
| `Text field` (`Style=Filled`) | B | M3 Design Kit | `text-field` · filled · label-floating · 56px | `<igc-input>` | high | Close with `input-group` tokens |
| `Frame 427` | C | — | `tag` (pill, 24px) | `<igc-badge>` | low | Confirm with user |
| `_Button/Contained` | A | Indigo.Design (Material) | `button` · high | `<igc-button variant="contained">` | high | — |

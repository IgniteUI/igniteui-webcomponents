...
> project. Read the JSX to extract the information below. Asset URLs in the output (localhost on the desktop server, https on
...
3. From the React+Tailwind output, extract:
   ...
   - **Surface colors** — `bg-[#XXXXXX]` classes on container `<div>` elements that wrap major sections
     (these become plain `<div>` wrappers in the view, not Ignite UI components)
   ...
   - **Input type variants** *(Tier A only)* — look for hidden zero-size nodes (`size-[0.5px]`)
     ...
     underlined, label floating or above). Web Components expose this as the boolean
     `outlined` attribute, not a three-way type (SKILL.md Phase 4, rule 12).
...
2. Check for Code Connect mappings:
   ...
   **another library** (e.g. a shadcn kit connected to `@/components/ui/button`). Never copy
   their imports or tags: the target is always Ignite UI for Web Components.
...
- **Suspected Anatomy Deltas** lists only structural differences that tokens, documented
  `::part(...)` selectors, and slotted content cannot close. They are suspicions at this point: you only
...
> non-transparent background), it **must** have that background in the implementation.
...
and committed to the project's assets directory before Phase 4. Gradient placeholders are
not acceptable.
...
(`figma-component-map.md § Icons`, which also covers third-party kit icon sets).

...
export type IgcChatOptions = {
   ...
   * - `"below-messages"`: Renders suggestions below the last chat message.
   *
...
/**
 * @interface ChatMessageRenderer
 * Describes the contract for rendering a chat message.
 */
export interface ChatMessageRenderer {
  /**
   * Initializes the renderer. This method is called once when the renderer is set.
   * Useful for loading external resources (e.g., Shiki themes/languages).
   * @returns A Promise that resolves when initialization is complete.
   */
  init?(): Promise<void>;

  /**
   * Renders the given chat message content into a Lit TemplateResult.
   * @param message - The chat message to render.
   * @returns A Lit TemplateResult representing the rendered message.
   */
  render(message: IgcMessage): TemplateResult;
}

/**
 * Default implementation of ChatMessageRenderer for plain text.
 * This renderer simply displays the message as is, wrapped in a paragraph.
 */
export class PlainTextChatMessageRenderer implements ChatMessageRenderer {
  render(message: IgcMessage): TemplateResult {

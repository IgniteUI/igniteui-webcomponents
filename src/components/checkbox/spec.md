...
The label is normally the content projected in the default slot, which the component associates with the native
input inside its shadow root. When the label text has to live outside the component, set `aria-labelledby` on the
host and the component forwards the value to that input:

```html
<span id="external">External label</span>
<igc-checkbox aria-labelledby="external"></igc-checkbox>
<igc-switch aria-labelledby="external"></igc-switch>
```

Without the attribute, the inner input is labelled by the label element rendered inside the shadow root. Projecting
the label into the default slot stays the recommended option: an IDREF resolves within a single tree scope, so a
forwarded reference only takes effect when the referenced element is reachable from the inner input. Unlike the
input-shaped components of the library, the checkbox and the switch do not project external `label` elements through
`ElementInternals`.
...
- The switch suite has no validation message slot cases, although the component exposes the same slots through the
  shared base class.
- Neither suite runs an axe audit, asserts the `aria-labelledby` forwarding from the host, or covers the keyboard
  focus ring behavior.
...
- The projected label content is associated with the inner input, which provides the accessible name.
- An `aria-labelledby` attribute set on the host replaces that association on the inner input, for a label that

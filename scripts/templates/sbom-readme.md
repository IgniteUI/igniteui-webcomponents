# Software Bill of Materials - Ignite UI for Web Components {{version}}

Generated {{generatedAt}} from the release build of [`igniteui-webcomponents`](https://www.npmjs.com/package/igniteui-webcomponents).

## Documents in this archive

| File | Format | Scope |
|---|---|---|
| `{{deliveredCycloneDxFile}}` | CycloneDX 1.6 JSON | Delivered - every component resolved by `npm install igniteui-webcomponents` |
...
The **delivered** document describes exactly what a consuming application resolves: the `igniteui-webcomponents` package plus the transitive closure of its runtime `dependencies` and the `peerDependencies` it requires at runtime. Development-only packages are excluded, because they are never installed by consumers and never reach a production bundle. This is the document to feed a vulnerability scanner or a compliance review of the shipped product.
...
| Version | `{{version}}` |
| PURL | `pkg:npm/igniteui-webcomponents@{{version}}` |
| File | `{{tarballName}}` |
| SHA-256 | `{{sha256}}` |
| SHA-512 | `{{sha512}}` |

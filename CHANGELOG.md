...
- #### Virtual grid
  ...
  - `rowTemplate` renders a whole row, for example a group header, as one cell that spans every column and stays in view during a horizontal scroll. `autoSizeColumn` sets a column to the widest of its rendered cells. With `auto-row-height`, the scroll offset follows a row that is measured above the viewport, thus the rows in view do not jump.
- #### Virtual scroll

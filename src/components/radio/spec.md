...
<igc-radio aria-labelledby="external"></igc-radio>
```
...
input-shaped components of the library, the radio do not project external `label` elements through
`ElementInternals`.
...
[`radio-group.spec.ts`](../radio-group/radio-group.spec.ts), not in this suite. The suite also runs no axe audit.
## Accessibility

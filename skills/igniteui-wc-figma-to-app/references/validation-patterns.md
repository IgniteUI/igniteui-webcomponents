...
| **Accepted** | Recorded anatomy delta     | The difference matches a Phase 2d delta-ledger entry the user approved (e.g. an M3 segmented button's check icon, a sheet rendered as a dialog) | Report only. Do not "fix" it, and it does not count toward the 3-retry rule |

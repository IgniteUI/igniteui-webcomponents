...
- #### Icon
  - An icon rendered through a reference stayed blank when the icon it points to was registered afterwards, because the component only reacted to registrations matching its own `name` and `collection` rather than the resolved target. It now re-resolves on any registry change.
  - `igc-icon` now declares `role="img"` only when it has an accessible name - the icon's own `<title>`, or an `aria-label` / `aria-labelledby` on the host. A decorative icon previously appeared in the accessibility tree as an unnamed image.
- #### Select
